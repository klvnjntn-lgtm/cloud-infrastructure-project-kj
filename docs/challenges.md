...
Grafana failed to establish a connection to the database during deployment.

Impact:

Grafana service was unable to start successfully, blocking observability stack initialization.

Root Cause:

Missing required environment variable for PostgreSQL SSL configuration:

```bash
{ name = "GF_DATABASE_SSL_MODE", value = "require" }
```

Without this, the database connection was rejected due to SSL enforcement.
