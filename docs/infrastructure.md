...
CloudWatch + Grafana (metrics + logs)
...
Observability is centralized to improve detection speed and simplify cross-service debugging across ECS, ALB, and RDS.
...
Used for debugging application crashes, deployment failures, and runtime errors

Alerts

Triggered on key system degradation signals:

High ECS CPU or memory usage

ALB unhealthy targets

RDS latency or connection spikes
...
Enables faster detection and correlation of failures across compute, network, and database layers, reducing MTTR and improving operational visibility.
